---
name: percolate-oh
description: Review a hraness/oh Markdown vault for recurring ideas and missing structural connections, then promote evidence-backed concepts and typed relationships with the Oh CLI. Use after materially adding or revising notes, when organizing an accumulated vault, or when an agent needs to turn repeated tags and prose references into an explicit queryable knowledge graph.
---

# Percolate concepts and relationships

Keep the graph authored, local, and reviewable. `oh percolate` proposes
...
```sh
oh percolate notes/example --root "$OH_ROOT" --limit 25 --json
```

Run it without a note only when reviewing the whole vault:

```sh
oh percolate --root "$OH_ROOT" --min-support 2 --limit 50 --json
```
...
natural ID is already an ordinary note, so Oh chooses an unoccupied
...
```sh
oh note create notes/local-first \
  --root "$OH_ROOT" \
  --title "Local-first" \
  --type concept \
  --tag architecture \
...
```sh
oh percolate notes/write-path --root "$OH_ROOT" --limit 25 --json
```
...
```sh
oh relation list notes/write-path --root "$OH_ROOT" --json
oh relation remove notes/write-path supports notes/durable-agent-memory \
  --root "$OH_ROOT"
```
...
```sh
oh links notes/write-path --root "$OH_ROOT" --direction both --depth 2 --json
oh datalog '[:find ?target :where [?edge :edge/predicate "supports"] [?edge :edge/target ?target-ref] [?target-ref :note/id ?target]]' \
  --root "$OH_ROOT" --json
```
