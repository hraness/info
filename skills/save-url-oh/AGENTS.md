...
- Invoke the installed `oh` CLI; do not depend on a source checkout or implementation path.
...
- Verify the installed environment with `oh doctor` and the current platform matrix with `oh adapters`.
