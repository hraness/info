...
These examples assume `OH_ROOT` is the resolved vault directory containing the managed `index.md`.
...
oh clip https://example.com/member/article --browser-profile "Work" --output "$OH_ROOT/articles"
```
...
```sh
oh clip https://example.com/member/article --browser-live --output "$OH_ROOT/articles"
oh clip https://example.com/member/article --cdp 9222 --output "$OH_ROOT/articles"
```

The external browser remains open. Choose `oh clip current` instead when the already-open view is the source of truth.
...
oh clip https://example.com/member/article --cookie-source firefox --cookie-profile "work" --output "$OH_ROOT/articles"
...
```sh
oh clip current --browser-live --cookie-source chrome --cookie-profile "Default" --media all --output "$OH_ROOT/articles"
```
...
```sh
oh clip https://example.com/member/article --html "$OH_SAVED_HTML" --output "$OH_ROOT/articles"
oh clip https://example.com/member/article --html - --output "$OH_ROOT/articles" < page.html
```
