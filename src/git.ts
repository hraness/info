...
function queryTerms(query: string): { readonly normalized: string; readonly terms: readonly string[] } {
...
export function searchGitHistory(
  ...
  const allowed = options.allowedNoteIds === undefined
    ? null
    : new Set(options.allowedNoteIds);
  ...
  for (const note of index.notes) {
    if (allowed !== null && !allowed.has(note.id)) continue;
